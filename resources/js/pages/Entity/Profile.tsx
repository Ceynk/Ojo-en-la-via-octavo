...
interface EntityProfilePageProps extends PageProps {
...
export default function EntityProfile() {
    const { user, stats, recent_activity, teammates } = usePage<EntityProfilePageProps>().props;
    ...
    return (
        <EntityLayout breadcrumb="Mi perfil">
            ...
            {activity.length === 0 ? (
                ...
                <ul className="space-y-3">
                    {activity.map((h) => (
                        <li key={h.id}>
                            <Link
                                href={`/entidad/reportes/${h.report_id}`}
...
function PreferencesCard({ notifyByEmail }: { notifyByEmail: boolean }) {
    ...
    function toggle() {
        if (toggling) return;
        setToggling(true);
        router.put('/entidad/perfil/preferencias', { notify_by_email: !notifyByEmail }, {
            preserveScroll: true,
...
function SecurityCard() {
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        form.put('/entidad/perfil/password', {
            preserveScroll: true,
            onSuccess: () => {
                setChanging(false);
                form.reset();
